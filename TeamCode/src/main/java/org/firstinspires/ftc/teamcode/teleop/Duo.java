...
//            telemetry.addData("Right stick Val:", gamepad2.right_stick_y);
            ...
            // Lifts --> Right Stick Y
            leftlift.setPower(gamepad2.right_stick_y * armmodifier);
            rightlift.setPower(-gamepad2.right_stick_y * armmodifier);

            // Move arm from Horizontal to Vertical --> Triggers
            if (gamepad2.right_trigger > 0) {
                raisemotor.setPower(0.2 + (0.65 * gamepad2.right_trigger));
            } else if (gamepad2.left_trigger > 0) {
                raisemotor.setPower(-(0.2 + (0.65 * gamepad2.left_trigger)));
            } else raisemotor.setPower(0);

            // Claw Control --> a and b
            if (gamepad2.b) {claw.setPosition(0.55);}
            if (gamepad2.a) {claw.setPosition(0.3);}

            // Wrist Control --> DPAD
            if (gamepad2.dpad_up) {leftwrist.setPosition(0.0); rightwrist.setPosition(0.0);}
            if (gamepad2.dpad_down) {leftwrist.setPosition(0.7); rightwrist.setPosition(0.7);}
            if (gamepad2.dpad_left) {leftwrist.setPosition(0.3); rightwrist.setPosition(0.3);}
            if (gamepad2.dpad_right) {leftwrist.setPosition(0.45); rightwrist.setPosition(0.45);}

            // Arm Locks --> x and y
            if (gamepad2.x) {leftlock.setPosition(0.7); rightlock.setPosition(0.2);}
            if (gamepad2.y) {leftlock.setPosition(1); rightlock.setPosition(0.5);}
